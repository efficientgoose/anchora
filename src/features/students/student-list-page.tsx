...
import { countRisks, initials, nextTask, progressFor, relativeDateLabel, studentRisk } from "@/domain/student-calculations";
import { demoClock } from "@/domain/clock";
import { cn } from "@/lib/cn";
import { RiskBadge, riskMeta } from "./risk-ui";
import { useAllStudents, useStudents } from "./data";
...
function StudentCard({ student }: { student: Student }) {
  ...
  return (
    <Link href={`/students/${student.id}`} className="block rounded-xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,.04)] transition hover:border-slate-300">
...
export function StudentListPage() {
  ...
  const studentsQuery = useStudents(query); const allQuery = useAllStudents();
  const allStudents = allQuery.data ?? []; const counts = countRisks(allStudents, demoClock);
  ...
  return (
    <div className="mx-auto max-w-[1340px] p-4 sm:p-6 lg:px-8 lg:py-7 page-enter">
      <header className="mb-[22px] flex items-start justify-between gap-4"><div><h1 className="text-2xl font-semibold tracking-[-.025em]">Students</h1><p className="mt-1 text-sm text-slate-500">{allStudents.length} active students · {counts.overdue + counts.atRisk} need attention</p></div><Button asChild><Link href="/students/new"><Plus /> Add Student</Link></Button></header>
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-wrap items-center gap-2.5 border-b border-slate-100 p-3.5 sm:px-4">
          <div className="relative min-w-[210px] flex-1 sm:max-w-[236px]"><Search className="absolute left-3 top-1/2 size-3.5 -translate-y-1/2 text-slate-400" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search students…" className="h-[34px] pl-9 text-[13px]" aria-label="Search students" /></div>
          <Select ariaLabel="Filter by risk" value={query.risk} onValueChange={(value) => update({ risk: value })} className="h-[34px] w-[120px] text-[13px]" options={[{value:"all",label:"All Risk"},{value:"overdue",label:"Overdue"},{value:"at_risk",label:"At Risk"},{value:"on_track",label:"On Track"}]} />
          <Select ariaLabel="Filter by consultant" value={query.consultantId} onValueChange={(value) => update({ consultant: value })} className="h-[34px] w-[160px] text-[13px]" options={[{value:"all",label:"All Consultants"},...DEMO_STAFF.map((member) => ({value:member.id,label:member.name}))]} />
          <Select ariaLabel="Filter by intake" value={query.intake} onValueChange={(value) => update({ intake: value })} className="h-[34px] w-[140px] text-[13px]" options={[{value:"all",label:"All Intakes"},...INTAKES.map((intake) => ({value:intake,label:intake}))]} />
          ...
          <span className="ml-auto text-xs text-slate-400">{studentsQuery.data?.total ?? 0} students</span>
        </div>
