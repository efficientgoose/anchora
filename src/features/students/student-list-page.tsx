  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-[1340px] overflow-x-hidden p-4 sm:p-6 lg:px-8 lg:py-7 page-enter">
      ...
      <Card className="overflow-hidden">
        <div className="grid grid-cols-2 items-center gap-2 border-b border-slate-100 p-3 sm:flex sm:flex-wrap sm:px-4">
          <div className="relative col-span-2 w-full sm:max-w-[220px] sm:flex-1"><Search className="absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-slate-400" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search students…" className="h-[30px] rounded-md py-0 pl-8 pr-2.5 text-xs" aria-label="Search students" /></div>
          <Select ariaLabel="Filter by risk" value={query.risk} onValueChange={(value) => update({ risk: value })} className="h-[30px] w-full rounded-md px-2.5 text-[11px] leading-none sm:w-[112px]" options={[{value:"all",label:"All Risk"},{value:"overdue",label:"Overdue"},{value:"at_risk",label:"At Risk"},{value:"on_track",label:"On Track"}]} />
          <Select ariaLabel="Filter by consultant" value={query.consultantId} onValueChange={(value) => update({ consultant: value })} className="h-[30px] w-full rounded-md px-2.5 text-[11px] leading-none sm:w-[148px]" options={[{value:"all",label:"All Consultants",shortLabel:"Consultants"},...DEMO_STAFF.map((member) => ({value:member.id,label:member.name}))]} />
          <Select ariaLabel="Filter by intake" value={query.intake} onValueChange={(value) => update({ intake: value })} className="h-[30px] w-full rounded-md px-2.5 text-[11px] leading-none sm:w-[128px]" options={[{value:"all",label:"All Intakes"},...INTAKES.map((intake) => ({value:intake,label:intake}))]} />
          {hasFilters && <button onClick={() => { setSearch(""); router.replace(pathname); }} className="px-1 text-xs font-medium text-brand-charcoal hover:underline">Clear filters</button>}
          <span className="ml-auto text-right text-xs text-slate-400">{studentsQuery.data?.total ?? 0} students</span>
        </div>
        {studentsQuery.isLoading ? <div className="space-y-px bg-slate-100 p-px">{Array.from({length:6}).map((_, index) => <div key={index} className="h-16 animate-pulse bg-white" />)}</div> : studentsQuery.isError ? <div className="p-12 text-center"><SlidersHorizontal className="mx-auto size-8 text-slate-300" /><p className="mt-3 text-sm font-medium">Students could not be loaded</p><button onClick={() => studentsQuery.refetch()} className="mt-2 text-sm font-medium text-brand-charcoal hover:underline">Try again</button></div> : !studentsQuery.data?.items.length ? <div className="p-12 text-center"><UserRound className="mx-auto size-9 text-slate-300" /><p className="mt-3 text-sm font-medium">No students match these filters</p><p className="mt-1 text-xs text-slate-400">Clear a filter or try a different search.</p></div> : <>
          <div className="hidden overflow-x-auto md:block"><div className="min-w-[900px]"><div className="grid grid-cols-[104px_minmax(180px,1.5fr)_110px_minmax(190px,1.4fr)_minmax(150px,1fr)_130px] items-center gap-4 border-b border-slate-200 px-5 py-3 text-[11px] font-semibold uppercase tracking-[.04em] text-slate-500">{table.getHeaderGroups()[0].headers.map((header) => <div key={header.id}>{flexRender(header.column.columnDef.header, header.getContext())}</div>)}</div>{table.getRowModel().rows.map((row) => { const risk = studentRisk(row.original, demoClock); return <Link href={`/students/${row.original.id}`} key={row.id} className={cn("grid grid-cols-[104px_minmax(180px,1.5fr)_110px_minmax(190px,1.4fr)_minmax(150px,1fr)_130px] items-center gap-4 border-b border-l-[3px] border-b-slate-100 px-5 py-3 transition hover:bg-slate-50", risk === "overdue" ? "border-l-red-600" : risk === "at_risk" ? "border-l-amber-500" : "border-l-green-600")}>{row.getVisibleCells().map((cell) => <div key={cell.id} className="min-w-0">{flexRender(cell.column.columnDef.cell, cell.getContext())}</div>)}</Link>; })}</div></div>
