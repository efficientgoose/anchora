...
function PortalTask({ task }: { task: ApplicationTask }) {
  ...
  return (
    <div className="flex items-center gap-3.5 border-b border-slate-100 px-5 py-3.5 last:border-b-0">
      ...
      <div className="min-w-0 flex-1"><div className={cn("text-sm font-medium", done ? "text-slate-400 line-through" : "text-brand-charcoal")}>{task.name}</div><div className="text-xs text-slate-400">{done ? `Completed ${formatDate(task.dueDate)}` : formatDate(task.dueDate)}</div></div><span className={cn("text-xs font-semibold", status.text)}>{status.label}</span>
    </div>
  );
}
...
export function StudentPortalPage({ studentId }: { studentId: string }) {
  ...
  if (!query.data) return <div className="flex min-h-screen flex-col items-center justify-center"><GraduationCap className="size-10 text-slate-300" /><h1 className="mt-4 text-lg font-semibold">Portal not found</h1><Link href="/students" className="mt-3 text-sm font-medium text-brand-charcoal hover:underline">Back to consultant view</Link></div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-[720px] space-y-5 px-4 py-9 sm:px-6 sm:py-10 page-enter">
        ...
        {upcoming && <Card className="flex items-center gap-4 p-5"><span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-brand-gold/15 text-brand-charcoal"><CircleCheck className="size-5" /></span><div className="min-w-0 flex-1"><div className="text-xs font-medium uppercase tracking-[.05em] text-slate-400">Your next step</div><div className="mt-1 font-semibold">{upcoming.name}{upcoming.university ? ` · ${upcoming.university}` : ""}</div><div className="text-xs text-slate-500">{formatDate(upcoming.dueDate)} · {relativeDateLabel(upcoming.dueDate, demoClock)}</div></div></Card>}
