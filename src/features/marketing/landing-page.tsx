      ...
      <header className="sticky top-0 z-40 border-b border-slate-100 bg-white/85 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1140px] items-center px-5 sm:px-7">
          ...
          <nav className="ml-11 hidden items-center gap-7 text-sm text-slate-600 md:flex" aria-label="Main navigation">
            <a className="transition hover:text-indigo-600" href="#product">Product</a>
            <a className="transition hover:text-indigo-600" href="#how">How it works</a>
            <a className="transition hover:text-indigo-600" href="#why">Why Anchora</a>
          </nav>
      ...
      <main>
        <section className="mx-auto grid max-w-[1140px] items-center gap-14 px-5 py-16 sm:px-7 lg:grid-cols-2 lg:py-[76px]">
          <div className="page-enter">
            <div className="mb-[22px] inline-flex items-center gap-2 rounded-full border border-indigo-100 bg-indigo-50 px-3 py-1.5 text-[12.5px] font-semibold text-indigo-700">
              <span className="size-1.5 rounded-full bg-indigo-600" />Built for study-abroad consultancies
            </div>
            <h1 className="max-w-[520px] text-[42px] font-bold leading-[1.06] tracking-[-.035em] text-slate-950 sm:text-5xl">Every applicant on track for Germany.</h1>
              ...
              <div className="flex flex-col gap-[9px] p-4">
                {previewRows.map((row) => (
                  <div key={row.name} className={`flex items-center gap-3 rounded-[10px] border border-slate-100 border-l-[3px] bg-[#fbfcfe] px-3 py-2.5 ${row.border}`}>
                    <span className="flex size-[30px] shrink-0 items-center justify-center rounded-full bg-indigo-50 text-[11px] font-semibold text-indigo-600">{row.initials}</span>
        ...
        <section id="why" className="border-y border-slate-100 bg-[#fbfcfe]">
          <div className="mx-auto grid max-w-[1140px] grid-cols-2 gap-6 px-5 py-8 sm:px-7 lg:grid-cols-4">
            {[["3s", "To spot who's at risk"], ["0", "Spreadsheets to maintain"], ["100%", "Of deadlines tracked live"], ["1 day", "To move your whole caseload"]].map(([value, label]) => (
              <div key={label}><div className="text-[30px] font-bold tracking-[-.025em] text-indigo-600">{value}</div><div className="mt-0.5 text-[13px] text-slate-500">{label}</div></div>
            ))}
          </div>
        </section>
        ...
        <section id="product" className="mx-auto max-w-[1140px] px-5 pb-10 pt-20 sm:px-7">
          <div className="mb-11 max-w-[640px]">
            <div className="mb-3 text-[13px] font-semibold uppercase tracking-[.06em] text-indigo-600">Everything in one place</div>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {features.map(({ icon: Icon, title, body }) => (
              <article key={title} className="rounded-[14px] border border-slate-200 bg-white p-6 transition duration-300 hover:-translate-y-1 hover:shadow-[0_16px_32px_-20px_rgba(15,23,42,.3)]">
                <span className="mb-4 flex size-[42px] items-center justify-center rounded-[11px] bg-indigo-50 text-indigo-600"><Icon className="size-5" /></span>
        ...
        <section id="how" className="mx-auto max-w-[1140px] px-5 pb-20 pt-14 sm:px-7">
          <div className="grid items-center gap-12 overflow-hidden rounded-[20px] bg-slate-950 px-7 py-10 text-slate-200 sm:p-12 lg:grid-cols-[1.1fr_1fr]">
            <div>
              <div className="mb-3.5 text-[13px] font-semibold uppercase tracking-[.06em] text-indigo-300">The old way vs Anchora</div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="rounded-xl border border-indigo-600 bg-slate-800 p-4"><div className="mb-2.5 text-xs font-semibold uppercase tracking-[.05em] text-indigo-300">With Anchora</div><div className="text-sm leading-[1.9] text-slate-200">One dashboard · Risk-sorted caseload · Student portal · Auto checklists</div></div>
            </div>
          </div>
        </section>
      </main>
